 ...
 * Sypex Geo is released under the terms of the BSD license http://sypex.net/bsd_license.txt.
 */
class SxGeo
{
    const SXGEO_FILE = 0;
    ...
    public function __construct($db_file = 'SxGeo.dat', $type = self::SXGEO_FILE)
    {
        ...
        if ('SxG' !== \mb_substr($header, 0, 3)) {
            die("Can't open {$db_file}\n");
        }
        $info = \unpack('Cver/Ntime/Ctype/Ccharset/Cb_idx_len/nm_idx_len/nrange/Ndb_items/Cid_len/nmax_region/nmax_city/Nregion_size/Ncity_size/nmax_country/Ncountry_size/npack_size', \mb_substr($header, 3));
        ...
        if ($this->batch_mode) {
            ...
            $this->m_idx_arr = \mb_str_split($this->m_idx_str, 4); // Быстрее в 5 раз чем с циклом
    ...
    protected function search_idx($ipn, $min, $max)
    {
        if ($this->batch_mode) {
            ...
            while ($max - $min > 8) {
                $offset = ($min + $max) >> 1;
                if ($ipn > \mb_substr($this->m_idx_str, $offset * 4, 4)) {
                    $min = $offset;
                } else {
                    $max = $offset;
                }
            }
            while ($ipn > \mb_substr($this->m_idx_str, $min * 4, 4) && $min++ < $max) {
            }
        }
    ...
    protected function search_db($str, $ipn, $min, $max)
    {
        if ($max - $min > 1) {
            ...
            while ($max - $min > 8) {
                $offset = ($min + $max) >> 1;
                if ($ipn > \mb_substr($str, $offset * $this->block_len, 3)) {
                    $min = $offset;
                } else {
                    $max = $offset;
                }
            }
            while ($ipn >= \mb_substr($str, $min * $this->block_len, 3) && ++$min < $max) {
            }
        } else {
            ++$min;
        }

        return \hexdec(\bin2hex(\mb_substr($str, $min * $this->block_len - $this->id_len, $this->id_len)));
    }
    ...
    public function get_num($ip)
    {
        ...
        if ($this->batch_mode) {
            ...
            $blocks = \unpack('Nmin/Nmax', \mb_substr($this->b_idx_str, ($ip1n - 1) * 4, 8));
        }
    ...
    protected function readData($seek, $max, $type)
    {
        ...
        if ($seek && $max) {
            if ($this->memory_mode) {
                $raw = \mb_substr(1 === $type ? $this->regions_db : $this->cities_db, $seek, $max);
            } else {
    ...
    protected function unpack($pack, $item = '')
    {
        ...
        foreach ($pack as $p) {
            ...
            switch ($type0) {
                ...
                case 'M': $l = 3; break;
                case 'd': $l = 8; break;
                case 'c': $l = (int) \mb_substr($type, 1); break;
                case 'b': $l = \mb_strpos($item, "\0", $pos) - $pos; break;
                default: $l  = 4;
            }
            $val = \mb_substr($item, $pos, $l);
            switch ($type0) {
